resource "aws_ecs_service" "tf_ecs_service_ms1" {
  name            = "${var.project_name}-${var.microservices_name}-${var.project_environment}-service"
  task_definition = "${aws_ecs_task_definition.tf_ecs_task_definition_ms1.arn}"
  cluster         = "${var.ecs_cluster_id}"
  launch_type     = "FARGATE"
  desired_count   = "${var.desired_tasks}"

  network_configuration {
    security_groups  = ["${aws_security_group.tf_ecs_service_ms1_sg.id}"]
  ...
  depends_on = ["aws_alb_target_group.tf_alb_tg_ms1"]
}